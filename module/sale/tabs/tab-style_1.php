<input type="hidden" name="style" value="style_1" id="style">
<?php
$inputs = array(
	array(
		'field' => 'bg_color', 'label' => 'Màu nền', 'type'  => 'color'
	),
	array(
		'field' => 'border_color', 'label' => 'Màu viền', 'type'  => 'color'
	),
	//text option
	array(
		'field' => 'text', 'label' => 'Chữ', 'type'  => 'text'
	),
	array(
		'field' => 'text_color', 'label' => 'Màu chữ', 'type'  => 'color'
	),
	array(
		'field' => 'text_hidden', 'label' => 'Ẩn / hiện phần trăm', 'type'  => 'switch'
	),
	array(
		'field' => 'text_position', 'label' => 'Vị trí chữ', 'type'  => 'select',
		'options' => array(
			'percent-text'     => 'Sau phần trăm',
			'text-percent'     => 'Trước phần trăm',
		)
	),

	//position
	array(
		'field' => 'position', 'label' => 'Vị Trí', 'type'  => 'select',
		'options' => array(
			'top-left'     => 'Phía trên bên trái',
			'top-right'    => 'Phía trên bên phải',
			'bottom-left'  => 'Phía dưới bên trái',
			'bottom-right' => 'Phía dưới bên phải',
		)
	),
	array(
		'field' => 'border_radius', 'label' => 'Bo Tròn', 'type'  => 'border_radius'
	),
);

foreach ($inputs as $input) {
	echo _form( $input, (isset($item[$input['field']])) ? $item[$input['field']] : '' );
}